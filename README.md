...
Invite copy-link is `{origin}/invites?invite={token}`. There is no `/login?invite=` alias. Identity list returns plaintext `token` for admin/owner while the row is `active`; the app builds the URL. Members see prefix/status/role/expiry, not the token. Create `max_uses` omitted = 1; JSON `null` = unlimited. Invite JSON uses `status` for lifecycle (`active` / `redeemed` / `revoked` / `expired`); no `redeemed_at` / `revoked_at` on the wire.

Already signed in: redeem immediately (skip PKCE). Else the app stashes the token in a first-party cookie (`plat5_web_demo_invite`, not Auth’s `plat5_invite_token`) plus an origin stash keyed by OAuth CSRF `state`, strips the query so Referer to Auth cannot leak it, starts PKCE (Auth `/authorize` does **not** get `invite=`; token never in OAuth `state`), then `POST /api/invites/redeem` with `{ "token" }` and the session JWT. Cookie/stash clear only after a successful redeem. Dead redeem is **409** `CONFLICT` with `details` `{ field: "status", value }` (`redeemed` | `revoked` | `expired`). Unknown token is **404**. Add-by-`user_id` still works. Email is unbound. No SMTP.
